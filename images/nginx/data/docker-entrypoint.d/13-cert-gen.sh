...
cert_gen_generate_ca() {
	...
	if [ ! -f "${key}" ] || [ ! -f "${crt}" ]; then
		...
		if ! run \
			"ca-gen -v -c VN -s Hanoi -l Hanoi -o SpaceDocker -u SpaceDocker -n 'Workspace Docker Root CA' -e 'workspace@nntoan.com' \"${key}\" \"${crt}\"" \
...
cert_gen_generate_cert() {
	...
	if ! run \
		"cert-gen -v -c VN -s Hanoi -l Hanoi -o SpaceDocker -u SpaceDocker -n '${cn}' -e 'admin@${cn}' -a '${alt_names}' \"${ca_key}\" \"${ca_crt}\" \"${key}\" \"${csr}\" \"${crt}\"" \
